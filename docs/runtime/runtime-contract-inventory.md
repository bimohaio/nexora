...
| `RuntimeVisualSnapshot`, `RuntimeVisualSnapshotDiff` | Runtime Engine | Immutable resolved state and incremental diff | Renderer consumers                | MINOR additive contract       |
| `RuntimeVisualCommitEvent`                           | Runtime Engine | Atomic previous/current snapshot delivery     | Engine subscribers                | MINOR additive contract       |
| `RuntimeSimulator`, `RuntimeUpdateSink`              | Runtime Engine | Explicit lifecycle; atomic batch output       | Demos, tests                      | MINOR addition                |
