...
test("symbol gallery renders the complete catalog and preview controls", async ({ page }) => {
  ...
  await expect(page.locator("article[data-symbol-type]")).toHaveCount(45);
  await expect(page.locator("[data-category]")).toHaveCount(14);
  await expect(page.locator("[data-scada-root]")).toHaveCount(45);
  ...
  await expect(page.locator("[data-scada-symbol]")).toHaveCount(45);
});
